...
const deletePerson = async (email) => {
    const response = await fetch(`${apiUrls.PERSON_DELETE_URL}${email}`, { method: 'DELETE' });
...
const searchPeople = async (query) => {
    const response = await fetch(`${apiUrls.PERSON_SEARCH_URL}${encodeURIComponent(query)}`);
...
const exportData = async (format) => {
    const url = format === 'json' ? apiUrls.PERSON_EXPORT_JSON_URL : apiUrls.PERSON_EXPORT_CSV_URL;
